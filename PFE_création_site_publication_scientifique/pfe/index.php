<?php
    ...
    //SUCCESS
    if(isset($_GET[sha1('user_added')])) $success = 'You are signed up, Verify your email!';
    if(isset($_GET[sha1('email_verified')])) $success = 'Your email is verified now, please sign in!';
    // echo "<script>window.history.pushState({}, document.title, '/pfe/');</script>";
	...
	if(isset($_POST['Newsletter']))
	{
		if(!empty($_POST["emailN"])){
			$bdd=new PDO("mysql:host=127.0.0.1;dbname=pfe;charset=utf8","root","");
...
<script src="scripts/fixer_menu.js"></script>
	...
	<script>
		...
		document.querySelector('#global-errors').addEventListener('click', ()=>{
			...
			window.history.pushState({}, document.title, "/pfe/");
		});
